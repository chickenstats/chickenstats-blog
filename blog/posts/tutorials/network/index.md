...
```
    ...
    PosixPath('/Users/scott/sports_blog/open_source/chickenstats-blog/nbs/blog/posts/tutorials/network/charts')
...
``` python
stats = scraper.prep_stats(level="season", teammates=True).stats.copy(deep=True)

stats_r = stats.loc[stats.session == "R"].copy()
```
...
``` python
plot_network(stats=stats_r, team=team, strengths=["5v5"], toi_min=15.0)
```
